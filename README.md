...
本アプリはデモ・学習用途を前提とし、実際の金銭決済、クレジットカード登録、銀行口座登録、本人確認書類の登録、実在する連絡先の保存は行わない。
...
| 実決済や個人情報保存によるリスクを避けたい | デモ認証、仮想ポイント、仮想ユーザー、サンプル画像、画面内通知で代替する |
...
| 認証 | デモユーザー、仮想セッション、開発用ロールで代替する | 実在メールアドレス必須化、大学認証の本番接続、本人確認書類アップロード |
| プロフィール | ニックネーム、学部、学年などの非本人特定情報だけを扱う | 氏名、住所、電話番号、生年月日、学生証番号の保存 |
...
ただし、代替実装であってもセキュリティ設計は本番に近い形で行う。認可チェック、入力値検証、CSRF/XSS対策、CORS制御、エラー情報の制限、操作ログの設計は、実データを扱う場合と同じ前提で設計する。
...
| 個人情報保護 | 実在する氏名、メールアドレス、電話番号、住所、学生証番号、カード情報などの入力・保存を求めない |
...
| 個人情報 | 氏名、実在メールアドレス、電話番号、住所、学生証番号、カード番号、銀行口座、本人確認書類の入力・保存を求めない |
...
本番の大学認証、実在メール確認、学生証アップロード、本人確認書類アップロードは実装しない。
...
| 個人情報非保存 | 実在メール、電話番号、住所、学生証番号、本人確認書類を入力・保存できない | 未確認 |
...
| セキュリティテスト | デモ認証、認可、SQLインジェクション、XSS、実決済遮断、個人情報非保存 |
...
| T-014 | 個人情報 | 実在メール、電話番号、住所、学生証番号 | 入力または送信する | 入力欄が存在しない、または400エラー |
