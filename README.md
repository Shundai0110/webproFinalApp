...
| 出品者 | 教科書を売りたい、または譲渡したいユーザー | 教科書投稿 / コメント対応 / 取引承諾・撤回 / 出品取り消し。frontendからの価格変更は**現状実装未定** |
| 購入者 | 教科書を探しているユーザー | 検索 / 詳細確認 / 購入リクエスト・取り消し / 取引承諾 |
...
- 成立前の購入リクエスト取り消し
- 購入者・出品者の双方承諾による取引成立
- 出品ステータスの変更（出品者による出品取り消し。出品編集のfrontend操作は**現状実装未定**）
...
backendに存在する画像URL、詳細検索、Book更新、アイコンURL、APIページングの各APIは利用可能だが、対応するfrontend操作は**現状実装未定**である。Book取り下げは、出品者が自分の`AVAILABLE`な出品を詳細画面から取り消す操作まで実装済みである。
...
| F-008 | 取引承諾 | 購入者・出品者が取引を承諾し、成立前は自分の承認を撤回する | 購入者 / 出品者 | 高 |
| F-009 | 取引成立判定 | 双方承諾済みの場合のみ取引を成立させる | システム | 高 |
| F-010 | ステータス管理 | 取引に伴う出品中、交渉中、売却済み、取り下げを管理する。出品者は`AVAILABLE`な自分の出品を取り消せる。frontendからの出品編集は**現状実装未定** | システム / 出品者 | 高 |
...
| API-008 | DELETE | `/api/books/:id` | 出品者が`AVAILABLE`な自分の出品を取り消す | F-010 |
| API-009 | POST | `/api/transactions` | 購入リクエストを送る | F-007 |
| API-010 | GET | `/api/transactions/:id` | 取引状態を取得する | F-008 |
| API-011 | PATCH | `/api/transactions/:id` | 取引を承諾する、または成立前に自分の承認を撤回する | F-008 / F-009 |
...
4. 自分自身の出品でないことを確認する。
5. 購入者の`PENDING`取引額を合計し、今回額との合計が現在残高を超える場合は`409 PURCHASE_BUDGET_EXCEEDED`で拒否する。
6. Transactionを`PENDING`として作成する。
7. Bookのstatusを`NEGOTIATING`に更新する。
8. この時点では実決済も仮想ポイント移動も行わない。
...
| 認証 | 必要 |
| 概要 | 購入者または出品者が取引を承諾するか、成立前に自分の承認を撤回する。購入者は成立前の購入申請自体も取り消せる。双方承諾済みなら取引を成立させる |
...
1. Transactionを取得する。
2. デモセッションのユーザーが操作可能な当事者であることを確認する。`CANCEL_PURCHASE`はbuyerだけに許可する。
3. `APPROVE`では、buyerなら`buyerApproved = true`、sellerなら`sellerApproved = true`にする。
4. `REVOKE_APPROVAL`では、`PENDING`中に限り操作ユーザー自身の承認をfalseに戻し、相手の承認は変更しない。未承認状態からの重複撤回は拒否する。
5. `CANCEL_PURCHASE`では、`PENDING`中に限りTransactionを`CANCELLED`、Bookを`AVAILABLE`に戻し、仮想ポイント残高は変更しない。
6. 更新後、`buyerApproved === true && sellerApproved === true`を確認する。
7. 両方trueの場合、Transactionを`COMPLETED`にする。
8. Bookを`SOLD`にする。
9. 仮想ポイント残高を更新する。
10. 画面内のデモ通知を作成する。
11. 完了画面用のレスポンスを返す。

`COMPLETED`後は承認撤回と購入申請取り消しを許可せず、取引・Book・残高を巻き戻さない。購入申請取り消し時のTransactionとBook、取引成立時のTransaction、Book、購入者・出品者残高、双方の `TRANSACTION_COMPLETED` 通知は、それぞれ選択中のstorageに対応する単一DBトランザクションで更新する。無料公開ではSQLite transaction、任意のローカルMySQLモードではPrisma transactionを使用する。`GET /api/notifications` は認証中ユーザー自身の通知だけを返す。
...
`backend/prisma/seed.ts` は5架空ユーザー、8架空Book、交渉中と成立済みのTransaction各1件、成立通知2件を冪等に作成する。実在メール、電話番号、住所、学生証・本人確認情報、認証情報、決済情報は含めない。既存のseedユーザーがいる場合はプロフィールだけを同期し、取引後の仮想ポイント残高は巻き戻さない。手動確認用の同等SQLは `database/seed.sql` に置く。
...
| BookForm | 教科書の出品フォーム。画像URL、編集操作は**現状実装未定** |
| BookDetail | 教科書詳細表示。出品者には`AVAILABLE`な自分の出品を取り消す操作を表示する |
| PurchaseButton | 購入リクエストを送る |
| TransactionAction | 取引承諾、成立前の自分の承認撤回、購入者による購入申請取り消しを行う |
...
| CANCELLED | 一覧には原則表示しない。`AVAILABLE`な自分の出品は詳細画面から取り消せる |
| Transaction PENDING | 双方承諾待ち表示。承認済みの当事者には自分の承認撤回、購入者には購入申請取り消しを表示 |
...
| 認可 | Book更新・取り消しは出品者のみ、Transaction承諾・承認撤回は取引当事者のみ、購入申請取り消しは購入者のみに許可する |
...
| API E2E | 2利用者のsession、出品共有、自己購入拒否、購入申請取り消し、双方承諾、ポイント移動、終了時reset |
| ブラウザE2E | 2つの独立browser contextによる出品共有、自分の出品表示、購入相談・取り消し |
...
| T-006-A | 購入リクエスト | PENDING総額と今回額の合計が現在残高を超える | 画面操作 / `POST /api/transactions` | 警告を表示し、`409 PURCHASE_BUDGET_EXCEEDED`で作成を拒否する |
| T-006-B | 購入リクエスト取り消し | buyerかつTransactionがPENDING | `PATCH /api/transactions/:id` | TransactionがCANCELLED、BookがAVAILABLEになり、残高は変わらない |
...
教科書詳細には、現在残高、申請中の購入額、今回の取引額、すべて成立した場合の残高を示す支払い風 UI を表示する。申請中額と今回額の合計が現在残高を超える場合は警告を表示して申請ボタンを無効化し、frontendを迂回したリクエストもbackendで拒否する。画面上の金額単位は利用者に分かりやすいよう`円`とし、プロフィールでは`円（デモ）`、支払い欄では`DEMO`と表示するが、内部では換金不可・現金価値なしの仮想ポイントとして扱う。ただし、カード番号、銀行口座、住所、電話番号などの入力欄や外部決済 API は持たず、実際の金銭や決済情報を扱わない。購入相談の作成時点では取引を `PENDING` として教科書を `NEGOTIATING` にするだけで、仮想ポイントは移動しない。

購入者は「購入・支払いを承諾」、出品者は「販売を承諾」をそれぞれ操作できる。`PENDING`中は各当事者が自分の承認だけを取り消せるが、相手の承認は変更しない。購入者は出品者の承認有無にかかわらず、`PENDING`中の購入申請を取り消せる。取り消すとTransactionを`CANCELLED`、教科書を`AVAILABLE`へ戻し、残高を変更せず申請中額から除外する。片方だけの承諾では `PENDING` のまま残高を変更せず、双方が承諾した場合だけ取引を `COMPLETED`、教科書を `SOLD` にし、購入者から出品者へ取引額分のデモ用仮想ポイントを移動して双方へ画面内通知を作成する。`COMPLETED`後の承認撤回・購入申請取り消し、第三者や出品者による購入申請取り消し、重複操作、残高不足での成立はAPI境界で拒否する。出品者は購入相談がない`AVAILABLE`な自分の出品を詳細画面から取り消せるが、`NEGOTIATING`または`SOLD`の出品は取り消せない。
