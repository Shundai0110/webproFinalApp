...
| 利用者 | 説明 | 主な操作 |
|---|---|---|
| 慶應生ユーザー | keio.jp認証またはkeio.jpメール確認済みの学生 | 教科書検索 / 出品 / 購入リクエスト / 承諾 |
...
| 区分 | 内容 |
|---|---|
| input | 教科書の写真、金額、説明、対象授業、使用年度、必須教材/参考書、学部、学科、対象学年 |
...
| 区分 | 内容 |
|---|---|
| input | 購入リクエスト、対象Book ID、購入者ID、金額、承諾操作 |
| logic | Transactionを作成し、購入者・出品者の承諾フラグを確認する。双方承諾済みの場合だけ取引成立にする |
| output | 購入完了画面を表示し、完了メールを送信する |
...
| 機能ID | 機能名 | 概要 | 利用者 | 優先度 |
|---|---|---|---|---|
| F-001 | アカウント登録 | keio.jpメールまたは大学認証により慶應生ユーザーを登録する | 慶應生ユーザー | 高 |
| F-002 | プロフィール表示・編集 | 氏名、学部、学科、学年、アイコン、ポイント残高を表示・編集する | 慶應生ユーザー | 中 |
| F-003 | 教科書出品 | 写真、価格、説明、授業名、使用年度などを登録する | 出品者 | 高 |
| F-004 | 教科書一覧表示 | 出品中の教科書を一覧表示する | 慶應生ユーザー | 高 |
| F-005 | 教科書検索 | 教科書名、授業、学部、学科、学年、年度、教材種別で検索する | 慶應生ユーザー | 高 |
| F-006 | 教科書詳細表示 | 出品情報、写真、価格、使用授業、使用年度、ステータスを表示する | 慶應生ユーザー | 高 |
...
| F-013 | 完了メール送信 | 取引成立後に購入者・出品者へ完了メールを送る | システム | 低 |
| F-014 | ポイント残高表示 | アプリ内ポイント残高を表示する | 慶應生ユーザー | 中 |
...
| 基本フロー | 1. 取引画面を開く<br>2. 購入者または出品者が承諾ボタンを押す<br>3. Express APIが該当する承諾フラグを更新する<br>4. buyer_approvedとseller_approvedを確認する<br>5. 両方trueならTransactionをCOMPLETEDにする<br>6. BookをSOLDにする<br>7. 完了画面を表示し、完了メールを送信する |
...
| セキュリティ | keio.jpメール確認または大学認証により利用者を制限する |
...
| 個人情報保護 | メールアドレスやプロフィール情報は必要最小限のみ保存する |
| 秘密情報管理 | DB接続情報や認証シークレットを環境変数で管理する |
| バックアップ | MySQLの永続化・スナップショット・バックアップ方針を定義する |
...
| UI-002 | アカウント登録画面 | `/register` | keio.jpメールまたは大学認証でユーザー登録する |
| UI-003 | プロフィール画面 | `/profile` | 学部、学科、学年、アイコン、ポイント残高を表示する |
...
| 使用年度 | text | - | 年度 |
| 出品者 | text | - | 出品者名 |
| 購入リクエストボタン | button | - | POST `/transactions`を呼び出す |
...
| 通信形式 | HTTPS |
| データ形式 | JSON |
| 認証方式 | keio.jpメール確認または大学認証。詳細方式は未決定 |
| 文字コード | UTF-8 |
...
|---|---|---|---|---|
| API-001 | POST | `/api/auth/register` | アカウント登録 | F-001 |
...
1. Transactionを取得する。
2. ログインユーザーがbuyerまたはsellerであることを確認する。
...
| email | VARCHAR(255) | - | - | ✓ | ✓ | keio.jpメール |
| year | INT | - | - | - | - | 学年 |
| faculty | VARCHAR(100) | - | - | - | - | 学部 |
| department | VARCHAR(100) | - | - | - | - | 学科 |
| icon_url | VARCHAR(1000) | - | - | - | - | アイコン画像URL |
| point_balance | INT | - | - | ✓ | - | アプリ内ポイント残高 |
...
| テーブル | インデックス名 | カラム | 目的 |
|---|---|---|---|
| users | unique_users_email | email | keio.jpメールの重複防止 |
...
| 対策 | 内容 |
|---|---|
| 慶應生限定 | keio.jpメール確認または大学認証により利用者を制限する |
| 認証情報管理 | セッションシークレットや認証関連情報を環境変数で管理する |
...
| 画像投稿 | ファイル種類、サイズ、保存先を制限する |
| エラー情報 | 本番環境では内部スタックトレースを返さない |
| ポイント管理 | 残高更新をDBトランザクション内で処理する |
| 個人情報 | メール、学部、学科、学年などの保存目的を明確にする |

### 13.1 keio.jp認証について

本設計では、慶應生限定の実現方法として以下を候補にする。

| 方式 | 内容 | 状態 |
|---|---|---|
| keio.jpメール確認 | `@keio.jp`などのメールアドレス宛に確認メールを送る | MVP候補 |
| 大学認証連携 | 大学側の認証基盤と連携する | 要確認 |
| 管理者承認 | 学生証などで手動確認する | 運用負荷が高いため後回し |

大学認証基盤の外部利用可否、OAuth/SAML等の方式、利用申請の要否は未確認である。  
MVPではkeio.jpメール確認から始める方針とする。
...
| `INVALID_KEIO_EMAIL` | 400 | keio.jpメールではない |
...
| `MAIL_FROM` | 完了メール送信用アドレス |
| `MAIL_API_KEY` | メール送信サービスを使う場合のAPIキー |
...
| テストID | 対象 | 条件 | 操作 | 期待結果 |
|---|---|---|---|---|
| T-001 | アカウント登録 | keio.jpメール | 登録する | Userが作成される |
| T-002 | アカウント登録 | keio.jp以外のメール | 登録する | 400エラー |
...
| ID | 内容 | 担当 | 期限 | 状態 |
|---|---|---|---|---|
| TBD-001 | keio.jp認証をメール確認で実装するか、大学認証連携にするか | 〇〇 | YYYY-MM-DD | 未決定 |
| TBD-002 | 実決済を入れるか、仮想ポイントのみにするか | 〇〇 | YYYY-MM-DD | 未決定 |
| TBD-003 | 画像ファイルの保存先をどこにするか | 〇〇 | YYYY-MM-DD | 未決定 |
| TBD-004 | コメント機能をMVPに含めるか、Phase2に回すか | 〇〇 | YYYY-MM-DD | 未決定 |
| TBD-005 | 授業マスタを作るか、Bookの文字列項目で始めるか | 〇〇 | YYYY-MM-DD | 未決定 |
| TBD-006 | MySQLをRender上で運用する際のバックアップ方法 | 〇〇 | YYYY-MM-DD | 未決定 |
| TBD-007 | 管理者機能をMVPに含めるか | 〇〇 | YYYY-MM-DD | 未決定 |
| TBD-008 | 完了メール送信サービスの選定 | 〇〇 | YYYY-MM-DD | 未決定 |
| TBD-009 | ポイント残高を実際の決済と結びつけるか | 〇〇 | YYYY-MM-DD | 未決定 |
