...
デモアカウントには `BUYER`（購入者）または `SELLER`（出品者）の固定ロールを設定する。出品は `SELLER`、購入相談は `BUYER` にだけ許可し、未認証状態と権限不足の操作は `frontend/src/apiClient.js` の更新処理で拒否する。UI のボタン無効化だけを認可判定として信用しない。プロフィールは編集できるが、ロールはコード内のデモアカウント定義から復元するため、`localStorage` のプロフィールを書き換えても権限は昇格しない。

出品者と購入者は変更不能なデモユーザー ID で識別する。同じユーザーが自分の出品へ購入相談を作成することは禁止し、別のデモユーザーに切り替えた場合だけ購入相談を作成できる。取引一覧は購入者または出品者になっているユーザーにだけ表示する。
