...
| 認証 | 必要 |
| 概要 | 購入者または出品者が取引を承諾するか、成立前に自分の承認を撤回する。双方承諾済みなら取引を成立させる |
...
`COMPLETED`後は承認を撤回できず、取引・Book・残高を巻き戻さない。現在のbackend実装では、Transaction、Book、購入者・出品者残高、双方の `TRANSACTION_COMPLETED` 通知を、選択中のstorageに対応する単一DBトランザクションで更新する。無料公開ではSQLite transaction、任意のローカルMySQLモードではPrisma transactionを使用する。`GET /api/notifications` は認証中ユーザー自身の通知だけを返す。
...
| ApprovalButton | 取引承諾と成立前の自分の承認撤回を行う |
...
| Transaction PENDING | 双方承諾待ち表示。承認済みの当事者には自分の承認を取り消す操作を表示 |
...
| 認可 | Book更新・取り消しは出品者のみ、Transaction承諾・承認撤回は購入者または出品者のみ許可する |
...
教科書詳細には、現在残高、取引額、成立後残高を示す支払い風 UI を表示する。画面上の金額単位は利用者に分かりやすいよう`円`とし、プロフィールでは`円（デモ）`、支払い欄では`DEMO`と表示するが、内部では換金不可・現金価値なしの仮想ポイントとして扱う。ただし、カード番号、銀行口座、住所、電話番号などの入力欄や外部決済 API は持たず、実際の金銭や決済情報を扱わない。購入相談の作成時点では取引を `PENDING` として教科書を `NEGOTIATING` にするだけで、仮想ポイントは移動しない。

購入者は「購入・支払いを承諾」、出品者は「販売を承諾」をそれぞれ操作できる。`PENDING`中は各当事者が自分の承認だけを取り消せるが、相手の承認は変更せず、`COMPLETED`後は撤回できない。片方だけの承諾では `PENDING` のまま残高を変更せず、双方が承諾した場合だけ取引を `COMPLETED`、教科書を `SOLD` にし、購入者から出品者へ取引額分のデモ用仮想ポイントを移動して双方へ画面内通知を作成する。取引当事者以外の承諾・撤回、同じ当事者による重複承諾・重複撤回、残高不足での成立は API 境界で拒否する。出品者は購入相談がない`AVAILABLE`な自分の出品を詳細画面から取り消せるが、`NEGOTIATING`または`SOLD`の出品は取り消せない。
