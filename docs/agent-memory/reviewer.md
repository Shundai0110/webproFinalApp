...
### 2026-07-31 双方承認後の承認取り消し確認

- 片側だけが承認した`PENDING`取引では、当事者が自分の承認を撤回できる。
- 2人目の承認処理内で即座にTransactionを`COMPLETED`、Bookを`SOLD`へ更新するため、「双方承認済みかつPENDING」の中間状態はない。
- `REVOKE_APPROVAL`は`PENDING`だけを受け付け、双方承認後は`409 TRANSACTION_CLOSED`で拒否する。成立済み取引の取消・返金は未実装である。
- `npm run test:e2e`が成功し、成立後の撤回拒否を確認した。製品コードは変更していない。

### 2026-07-31 複数購入申請時の残高確認

- 購入申請時は、その申請額と現在残高だけを比較しており、既存の`PENDING`取引額を予約・合算していない。
- 残高3,200円の利用者が1,800円と1,600円の2件、合計3,400円を申請できることを一時SQLiteで再現した。
- 取引成立時には残高を再確認するため、先に1,800円の取引が成立した後、残る1,600円の成立処理は`INSUFFICIENT_POINTS`で拒否された。残高がマイナスになることはない。
- 後から成立できない`PENDING`取引が残るため、申請時に未成立額を差し引いた利用可能残高を原子的に検査するか、予約残高と取引キャンセル・期限切れを実装する必要がある。

### 2026-07-31 開発振り返り文章の事実確認

- 初期commit `27270ec`のREADMEにも「6. 機能要件」と「7. 非機能要件」が存在するため、「設計時に作成しなかった」は事実と異なる。「例外フローや状態遷移まで十分に具体化しなかった」が正確である。
- `AGENTS.md`はAI用の設計書ではなく、READMEの仕様を実装へ反映するためのエージェント向け開発ガイドである。
- MakerとReviewerは別々のAIエージェントではない。同じCodex内で役割と参照する継続記録ファイルを分けた運用であり、完全に独立した内部記憶は保証しない。
- 旧実装の複数購入問題では承認変更そのものは可能だったが、残高不足で成立できない`PENDING`取引と`NEGOTIATING`のBookを購入者が取り消せず、取引を解消できなかった。
- 「AI要」は「AI用」、「デバック」は「デバッグ」、「対応出来ていない」は「対応できていない」へ修正する。
- 「人力でのデバッグで発見した」ことはリポジトリ記録だけでは断定できない。Reviewer評価、再現試験、追加テスト、ユーザー確認を含む「実装後のレビューや確認」とするのが安全である。
