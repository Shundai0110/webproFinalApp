...
frontend には複数のデモアカウント選択、期限付き仮想セッション、`BUYER` / `SELLER` ロール認可、プロフィール編集、教科書の簡易出品・検索・購入相談を実装しています。
...
出品は `SELLER`、購入相談は `BUYER` の固定ロールに限定します。UI の表示制御に加えて `frontend/src/apiClient.js` の更新関数でセッションとロールを再検証し、プロフィール保存値からのロール昇格を許可しません。
